...
def parse_args():
    ...
    # model
    parser.add_argument('--model', '-a', metavar='model', default='DRANet')
    # 输入通道数
...
# 训练函数
def train(config, train_loader, model, criterion, optimizer, deep_supervision=False, epoch=1):
    ...
    for input,target in pbar:
        ...
        if deep_supervision:
            ...
            loss = criterion(output[0], target.squeeze(1).long())
            out = output[0]
...
def validate(config, val_loader, model, criterion):
    ...
    with torch.no_grad():
        ...
        for input, target in pbar:
            ...
            output = model(input)
            loss = criterion(output[0], target.squeeze(1).long())
            iou = calc_iou(output[0], target, config['num_classes'])[1]
...
def main():
    ...
    # 获取数据
    train_root = "../kvasir_unet3plus/train"
    train_label = "../kvasir_unet3plus/train_label"
    ...
    val_root = "../kvasir_unet3plus/test"
    val_label = "../kvasir_unet3plus/test_label"
