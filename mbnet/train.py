...
def parse_args():
    ...
    # dataset
    parser.add_argument('--dataset', default='stomach_dataset08', help='dataset name')
...
# 训练函数
def train(config, train_loader, model, criterion,optimizer, deep_supervision=True, epoch=1):
    ...
    for input,target in pbar:
        ...
        output = model(input)
        if deep_supervision:
            # loss_ce = [criterion(out, target.squeeze(1).long()) for out in output]
            # loss_dice = [criterion_dice(out, target.squeeze(1).long(),softmax=True) for out in output]
            # loss_ce = sum(loss_ce) / len(loss_ce)
            # loss_dice = sum(loss_dice) / len(loss_dice)
            # out = output[0]
            # loss = 0.5*loss_ce+0.5*loss_dice
            loss = [criterion(out, target.squeeze(1).long()) for out in output]
...
def main():
    ...
    # 获取数据
    train_root = "../data08/train"
    train_label = "../data08/train_label"
    ...
    val_root = "../data08/test"
    val_label = "../data08/test_label"
    ...
    for epoch in range(config['epochs']):
        ...
        if val_log['iou'] > best_iou and val_log['iou'] > 0.75:
