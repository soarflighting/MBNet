...
def eval_semantic_segmentation(pred_labels, gt_labels,n_classes):
    ...
    * :math:`\\text{Mean Class Accuracy} = \\frac{1}{k} \
    ...
    iou = calc_semantic_segmentation_iou(confusion)     # iou 交并比
    pixel_accuracy = np.diag(confusion).sum() / confusion.sum()  # Accuracy 准确率
    class_accuracy = np.diag(confusion) / (np.sum(confusion, axis=1) + 1e-10)  #recall 召回率 + FPR
    precision = np.diag(confusion)/(np.sum(confusion,axis=0)+1e-10)            #precision 准确率
    iou_denominator = (confusion.sum(axis=1) + confusion.sum(axis=0))
    dice = 2*np.diag(confusion)/ iou_denominator

    return {'iou': iou, 'miou': np.nanmean(iou),
            'dice':dice,
            'precision':precision[1],
            'recall':class_accuracy[1],
            'fpr':class_accuracy[0],
